...
void Example::changeTitle( const godot::String ioString )
{
    auto *label = godot::Control::get_node<godot::Label>( "Title" );
    if ( label )
    {
        label->set_text( ioString );
    }
}
...
void Example::_process( double delta )
{
    if ( !url.is_empty() )
    {
        godot::UtilityFunctions::print( "Example::_process url->", url );
    }
}
...
void Example::_bind_methods()
{
    ...
    godot::ClassDB::bind_method( godot::D_METHOD( "changeTitle", "ioString" ),
                                 &Example::changeTitle );
    // ADD_PROPERTY( godot::PropertyInfo( godot::Variant::VECTOR2, "group_subgroup_custom_position"
    // ),
    //               "changeTitle", "get_custom_position" );
    ...
    godot::ClassDB::bind_method( godot::D_METHOD( "get_url" ), &Example::get_url );
    godot::ClassDB::bind_method( godot::D_METHOD( "set_url", "p_url" ), &Example::set_url );
    godot::ClassDB::add_property( "Example", godot::PropertyInfo( godot::Variant::STRING, "url" ),
                                  "set_url", "get_url" );
...
void Example::set_url( const godot::String p_url )
{
    if ( url != p_url )
    {
        url = p_url;
        godot::UtilityFunctions::print( "set_url->", url );
    }
}
godot::String Example::get_url() const
{
    return url;
}
